...
Progress:
	...
	Version 3.005
		31 January, 2017
		Fixing a bug which causes order number/team name textFrame to be appended on each mockup
			order number and team name are appended to the existing text frame instead of remaining as they are.

	Version 3.006
		1 February, 2017
		Adding the ability to press Enter to submit the dialog box.

	Version 3.007
		1 February, 2017
		Fixing a bug wherein garments that have multiple "Garment Code" textFrames the values for "Garment Code 2" were being changed when they were not supposed to.
			these frames were being added to the lib object and each subsequent artboard after the first one was pulling the garment code 2 info from the first artboard.
*/
...
function container()
{
	...
	function loopGarments()
	{
		...
		{
			thisLay = layers[a];
			infoLay = findSpecificLayer(thisLay.layers,"Information");
			if(infoLay)
			{
				if(!createDialog(infoLay,false))
				{
					return;
				}
			}
			...
			// 		{
			// 			if(!createDialog(thisSubLay,false))
			// 			{
			// 				return;
			// 			}
				...
				function submitFunction()
				{
					for (var fa in dialogGroups)
					{
						...
						else if(frameName == "Order Number")
						{
							...
							{
								lib["Order Number"] = inputText + " " +  dialogGroups["Team Name"]["input"].text;
						...
						else
						{
							...
							if(lib[frameName] == undefined && frameName.indexOf("Garment Description") < 0 && frameName.indexOf("Garment Code") < 0)
							{
