	...
	function createDialog(info,opts)
	{
		...
		var w = new Window("dialog", title);
			...
			for(var txt  = info.textFrames.length-1;txt  >-1; txt --)
			{
				...
				if(thisFrame.name == "Garment Code" || thisFrame.name == "Garment Description")
				{
					continue;
				}
				else if(thisFrame.name == "Fabric Type")
				{
					continue;
				}


				else if(thisFrame.name.indexOf("rder") > -1)
				{
			...
			//add submit, cancel, and moreOptions buttons
			...
			var btnGroup = w.add("group");
				...
				if(!opts)
				{
					...
					{
						for(var mo=0;mo<info.textFrames.length;mo++)
						{
							var thisFrame = info.textFrames[mo];
							if(thisFrame.name == "Garment Code" || thisFrame.name == "Garment Description")
							{
								var newGroup = w.add("group");
								hiddenFrames[thisFrame.name] = newGroup.add("statictext", undefined, thisFrame.name)
								hiddenFrames[thisFrame.name].characters = 20;
								hiddenFrameNames[thisFrame.name] = thisFrame.name;
							}
						}
				...
				else
				{
					...
					{
						for(var lo=0;lo<hiddenFrames.length;lo++)
						{
							var thisFrame = hiddenFrames[lo];
							hiddenFrames[lo].parent.remove(hiddenFrames[lo]);
							hiddenFrames[lo] = null;
						}
						hiddenFrames = [];
						w.layout.layout(true);
					}
